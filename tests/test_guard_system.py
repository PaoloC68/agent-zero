...
# ---------------------------------------------------------------------------
# 4. Cisco scaffold tests
# ---------------------------------------------------------------------------

class TestCiscoScaffold:

    def test_package_importable(self):
        import agent_zero_cisco_guard
        assert hasattr(agent_zero_cisco_guard, "skill_install_handler")

    def test_entry_point_discoverable(self):
        from importlib.metadata import entry_points
        eps = entry_points(group="agent_zero.guards")
        names = [ep.name for ep in eps]
        assert "cisco_scanner" in names

    def test_handler_skips_non_skill_install_events(self):
        from agent_zero_cisco_guard.handlers import skill_install_handler
        event = {"extension_point": "tool_execute_before", "tool_name": "test"}
        asyncio.get_event_loop().run_until_complete(
            skill_install_handler(event, agent=None)
        )
        # Should silently return without error

    def test_handler_warns_when_scanner_not_installed(self):
        from agent_zero_cisco_guard.handlers import skill_install_handler
        event = {
            "extension_point": "skill_install",
            "skill_name": "test-skill",
            "skill_path": "/tmp/test",
        }
        # skill_scanner is not installed, so handler should log warning and return
        asyncio.get_event_loop().run_until_complete(
            skill_install_handler(event, agent=None)
        )